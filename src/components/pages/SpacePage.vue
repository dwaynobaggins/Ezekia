...
The museum also works with partners and has set up an API where the museum website receives data related to one of these pages. For example, the museum partners with an observatory in Hawaii and receives
some Space highlights from them that need to be mixed in with the museum's own data.
...
Sample data is found in "Data()" and since we don't have any backend, pretend it was loaded in. Also to keep it simple, there are only a few highlights there, but there could be hundreds.
Note: The data is intentionally in different formats.
...
2.1.    Each page has a special badge in the top right corner of the card. This badge is different on every page, and could be an image, or some styled html elements, or a Font Awesome icon.
 The Space page has an image of a star. The Dinosaur page has a Font Awesome icon inside a circle. The Oceans page has two Font Awesome icons, a wave and a fish together. And so on...

2.2     The data on the different pages (Space, Dinosaurs...) have commonalities but also differences. All highlights have a name, image, date they were posted, a brief description, and perhaps associated news.
 But all pages also have extra unique data. For example, on the space page, some highlights also include a link to a quiz on the topic. 
 On the dinosaur card, we have a "Period" key to indicate when that dinosaur lived. On the Wildlife page, we have several additional keys: "Location", "Species", and "Endangered status".
...
<template>
    <div class="space-page">
        ...
        <div class="flex flex--wrap space-page__highlights">
            <MuseumHighlight v-for="(highlight, index) in sortedHighlights" @increment-picture-count="incrementPictureCount"
                :highlight="highlight"
                :index="index"
                type="Space"
                >
                <IconStar width="50" height="50" fill="#ED8A19"></IconStar>
            </MuseumHighlight>    
        </div>
    </div>
...
import MuseumHighlight from '../highlights/MuseumHighlight.vue';
import IconStar from '../icons/IconStar.vue';

export default {
    name: 'SpacePage',
    components: {
        MuseumHighlight, IconStar
    },
    mixins: [
    ],
    data() {
        return {
            ...
            partnerKeysTransformDict: {
                "createdAt": "date",
                "info": "description"
            },
            sortedHighlights: [],
            accumulatedPictureCount: 0,
        };
    },
    provide() {
        return {
            accumulatedPictureCount: computed(() => this.accumulatedPictureCount),
        }
    },
    methods: {
        incrementPictureCount() {
            this.accumulatedPictureCount++;
        }
    },
    created() {
        const spacePartnersKeysReduce = (a, v) => ({ ...a, [this.partnerKeysTransformDict[v] ?? v]: this.spacePartners.observatory[v]});
        // reduce is turning the array into a single object
        const spacePartnersObservatoryProcessedObj = Object.keys(this.spacePartners.observatory).reduce(spacePartnersKeysReduce, {});
        const labelledAsPartner = {...spacePartnersObservatoryProcessedObj, isPartner: true};

        const combinedArray = [...this.spaceHighlights, labelledAsPartner];

        this.sortedHighlights = orderBy(combinedArray, 'date', 'desc');
        this.accumulatedPictureCount = this.sortedHighlights.length - 1;
    },
};
</script>
